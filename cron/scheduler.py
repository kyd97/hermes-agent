...
def _compose_job_instruction(job: dict, *, include_cron_hint: bool = True) -> str:
...
def _build_job_prompt(job: dict) -> str:
    ...
    return _compose_job_instruction(job, include_cron_hint=True)
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    prompt = (
        _compose_job_instruction(job, include_cron_hint=False)
        if execution_mode == "delegate"
        else _build_job_prompt(job)
    )
    origin = _resolve_origin(job)
    ...
    try:
        ...
        if execution_mode == "delegate":
            _cron_future = _cron_pool.submit(_run_job_via_delegate, job, agent, prompt)
        else:
